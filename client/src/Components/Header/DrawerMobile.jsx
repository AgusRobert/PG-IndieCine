...
export default function DrawerMobile(infoUser) {
  ...
  const list = (anchor) => (
    <BoxA
      sx={{ width: anchor === 'top' || anchor === 'bottom' ? 'auto' : 400 , color:'black'}}
      role="presentation"
   
    >
      <List >
        {[ <OrderAZ />,<OrderDate />,<OrderRating />,<FilterMoviesByGenre />,<FilterMovieByCountry />,<FilterMovieByDuration />].map((comp, index) => (
